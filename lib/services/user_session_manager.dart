...
class UserSessionManager {
  ...
  /// 会话变更通知流
  final _sessionChangeController = StreamController<SessionChangeEvent>.broadcast();
  ...
  /// 会话变更事件流
  Stream<SessionChangeEvent> get sessionChanges => _sessionChangeController.stream;
  ...
  Future<void> startSession(String userId, WidgetRef? ref) async {
    if (_isSwitching) {
      debugPrint('[UserSession] Session switch already in progress, skipping...');
      return;
    }
    ...
    try {
      ...
      debugPrint('[UserSession] Session started successfully for user: $userId');
    } finally {
      _isSwitching = false;
    }
  }
  ...
  Future<void> _endSession(WidgetRef? ref, {required bool isLogout}) async {
    ...
    debugPrint('[UserSession] Ending session for user: $userId (logout: $isLogout)');
  ...
  Future<void> _invalidateProviders(WidgetRef ref, String userId) async {
    ...
    try {
      ...
      debugPrint('[UserSession] Providers invalidated (${arenaProviders.length + settingsProviders.length + tripProviders.length} total)');
