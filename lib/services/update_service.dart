...
class UpdateService {
  ...
  static Future<Map<String, String>> _selectFastestMirror(
    ...
    // 如果缓存有效（10分钟内），使用缓存结果
    if (_cachedMirrorResult != null && cacheAgeMinutes < _mirrorCacheValidMinutes) {
      debugPrint('✨ [Mirror] Using cached result (age: ${cacheAgeMinutes.toInt()} min)');
      ...
      return {
        'url': finalUrl,
        'name': cachedName,
        'fallbackUrl': _cachedMirrorResult!['fallbackUrl'] as String? ?? osgLabUrl,
        'fallbackName': _cachedMirrorResult!['fallbackName'] as String? ?? 'OSGLab镜像',
      };
    }
    ...
    for (var r in results) {
      if (r['available']) {
        debugPrint('   - ${r['name']}: ${r['speed'].toStringAsFixed(1)} KB/s (score: ${r['duration']})');
      } else {
    ...
    debugPrint('✅ Fastest: ${fastest['name']} (${fastest['speed'].toStringAsFixed(1)} KB/s)');
    debugPrint('⏱️ Fallback: ${fallback['name']} (${fallback.containsKey('speed') ? '${fallback['speed'].toStringAsFixed(1)} KB/s' : 'N/A'})');
    ...
    await prefs.setInt(_mirrorCacheTimeKey, DateTime.now().millisecondsSinceEpoch);
    debugPrint('💾 [Mirror] Result cached for ${_mirrorCacheValidMinutes} minutes');
  ...
  static Future<Map<String, dynamic>> _testMirrorSpeed(
    ...
    try {
      ...
      if (streamedResponse.statusCode == 200 || 
          streamedResponse.statusCode == 206) { // 206 = Partial Content
        ...
        // 计算速度 (KB/s)
        final speedKBps = durationMs > 0 
            ? (bytesDownloaded / 1024) / (durationMs / 1000)
            : 0;
        ...
        debugPrint('✅ [Mirror Test] $name OK: ${speedKBps.toStringAsFixed(1)} KB/s (score: $speedScore)');
        ...
        client.close();
        debugPrint('⚠️ [Mirror Test] $name returned ${streamedResponse.statusCode}');
        return {
          'url': url,
          'name': name,
          'duration': 999999,
              ...
              Expanded(
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    final officialWebsite = Uri.parse('https://puked.osglab.com/#download');
                    try {
                      if (await canLaunchUrl(officialWebsite)) {
                        await launchUrl(officialWebsite, mode: LaunchMode.externalApplication);
                      }
                      // 打开官网后关闭 App 里的更新弹窗
              ...
              if (snapshot.hasData) {
                ...
                switch (event.status) {
                  ...
                  case OtaStatus.INSTALLING:
                    ...
                    debugPrint('✅ [Update] Download completed, triggering installation manually');
  ...
  static Future<void> _triggerManualInstall(BuildContext context) async {
    try {
          ...
          try {
            ...
            for (var file in files) {
              if (file.path.endsWith('.apk')) {
                ...
                if (stat.size > 1024 * 1024) { // 大于 1MB 才是有效的 APK
                  apkPath = file.path;
                  debugPrint('📦 [Update] Found valid APK at: $apkPath (${(stat.size / 1024 / 1024).toStringAsFixed(1)} MB)');
                  break;
                }
              }
            }
      ...
      if (apkPath != null) {
        ...
        debugPrint('📊 [Update] open_file result: ${result.type}, message: ${result.message}');
        
        if (result.type != ResultType.done) {
          // 如果还是失败，可能是权限问题，尝试第二次使用不同方式
          debugPrint('🔄 [Update] Retrying installation with different approach...');
        }
      } else {
