...
class InertialNavigationEngine {
  ...
  void initialize(LatLng startPoint, Vector3 initialGyroBias, {double initialHeading = 0.0}) {
    _startLatLng = startPoint;
    _pos = Vector3.zero();
    _vel = Vector3.zero();
    
    // 根据初始航向初始化四元数 (假设初始 Pitch/Roll 为 0，由 SensorEngine 保证)
    _att = Quaternion.axisAngle(Vector3(0, 0, 1), initialHeading * math.pi / 180.0);
  ...
  void predict(SensorData data) {
    ...
    if (angleMag > 1e-9) {
      final Quaternion dq = Quaternion.axisAngle(deltaAngle.normalized(), angleMag);
      _att = _att * dq;
      _att.normalize();
    }
