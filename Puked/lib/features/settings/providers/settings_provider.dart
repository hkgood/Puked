...
class SettingsNotifier extends StateNotifier<SettingsState> {
  ...
  Future<void> _loadSettings() async {
    ...
    // 加载高帧率数据记录开关（已废弃：现在统一按平台最高帧率录制，此字段仅保留以兼容旧数据）
  ...
  /// 已废弃：帧率现在由平台自动决定（iOS 60fps / Android 30fps），无需手动开关。
  /// 保留此方法仅为向后兼容，调用无效果。
  @Deprecated('High frame rate is now always enabled per platform capability')
  Future<void> setHighFrameRateEnabled(bool enabled) async {
    // no-op: recording rate is now determined by platform automatically
  }
